...
ROOT = Path(__file__).parent.parent / "speakeasy_lowcost"
PCB = ROOT / "Speakeasy Lowcost.kicad_pcb"
OUTDIR = ROOT / "jlcpcb"
...
def main():
    ...
    export_gerbers(PCB, OUTDIR / "gerbers", OUTDIR / "Speakeasy_Lowcost_gerbers.zip")
    ...
    for variant in PANEL_VARIANTS:
        ...
        if panel_pcb.exists():
            ...
            export_gerbers(
                ...
                OUTDIR / f"Speakeasy_Lowcost_panel_{variant}_gerbers.zip",
            )
        else:
    ...
    print("Exporting CPL...")
    cpl_path = OUTDIR / "Speakeasy Lowcost_cpl.csv"
    run(
        [
            str(KICAD_CLI),
            "pcb",
    ...
    print(f"\nUpload to JLCPCB:")
    print(
        f"  Single board:  Speakeasy Lowcost_gerbers.zip + Speakeasy Lowcost_cpl.csv + speakeasy_lowcost_jlcpcb_bom.csv"
    )
    for variant in PANEL_VARIANTS:
        zip_name = f"Speakeasy_Lowcost_panel_{variant}_gerbers.zip"
